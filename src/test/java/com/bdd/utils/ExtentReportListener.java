...
public class ExtentReportListener implements EventListener{
	...
	*/
	...
	private void runStarted(TestRunStarted event) {
		System.out.println("in runStarted");
		spark = new ExtentSparkReporter("./extent-report/ExtentReportResults.html");
		spark.config().setDocumentTitle("BDD_PARALLEL_TESTING");
		spark.config().setReportName("BDD REPORT");
	...
	private void featureRead(TestSourceRead event) {
		System.out.println("in FeatureRead");
	...
	private void ScenarioStarted(TestCaseStarted event) {
		System.out.println("in ScenarioStarted");
		String featureName = event.getTestCase().getUri().toString();
		scenario = feature.get(featureName).createNode(event.getTestCase().getName());
		screenshotPath = "../extent-report/" + event.getTestCase().getName().replaceAll("\\s", "") + ".png";
	}
	
	// step started event
	// here we creates the test node
	private void stepStarted(TestStepStarted event) {
		System.out.println("in stepStarted");
		String stepName = " ";
	...
	private void stepFinished(TestStepFinished event) {
		if (event.getResult().getStatus().toString() == "PASSED") {
			...
			String err = event.getResult().getError().getMessage();
			try {
				step.log(Status.FAIL, "This failed: " + err, MediaEntityBuilder.createScreenCaptureFromPath(screenshotPath).build());
//				step.addScreenCaptureFromPath(screenshotPath);
//				step.log(Status.FAIL, "This failed: " + err);
			} catch (Exception e) {
